...
class BookingSearch
{
    ...
    public static function filter(Request $filters)
    {
        ...
        // Search for incoming bookings for waivers for specific date.
        if ($filters->has('waiverDate')){
            $arrivals->whereBetween('booking_data->arrival_date', [
                (new Carbon($filters->input('waiverDate')))->startOfDay()->toDateTimeString(),
                (new Carbon($filters->input('waiverDate')))->endOfDay()->toDateTimeString()
            ]);
        }
