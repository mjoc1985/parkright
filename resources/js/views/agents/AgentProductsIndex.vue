<template>
    <div class="flex-1" v-if="agent">
        <div class="card">
            <div class="card-header sm:p-4">
                <h1>{{agent.name}} - Products</h1>
                <router-link :to="{name: 'agents-products-create', params:{id:agent.id}}" class=" py-2 btn btn-primary">
                    <i class="fas fa-plus-circle mr-2"/>Create
                </router-link>
            </div>
            <div class="card-body p-0">
                <div>
                    ...
                    <div class="w-full p-8 text-center text-primary" v-if="products == null">There are no products to
                        show.
                    </div>
                </div>
            </div>
            <div class="card-footer p-4 flex font-semibold"></div>
        </div>
    </div>
</template>
<script>
